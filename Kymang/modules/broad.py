...
async def set_protect(c, m):
    ...
    if not adm and m.from_user.id != owner:
        return
    if len(m.command) < 2:
        return await m.reply("`/protect [True/False]`")
    jk = m.command[1]
    if jk in ["True", "False"]:
...
async def add_sub_bot(c, m):
    ...
    if not adm and m.from_user.id != owner:
        return
    if len(m.command) < 2:
        return await m.reply("Gunakan Format `/addbutton -100xxxx`")
    ids = int(m.command[1])
    adm = await sub_info(c.me.id, ids)
    await get_subs(c.me.id)
    s = await max_info(c.me.id)
    if len(m.command) == s:
        return await m.reply(
            f"Batas fsub {s} telah tercapai, Silahkan Hubungi Admin untuk bantuan."
        )
    if not adm:
        try:
...
async def del_sub_bot(c, m):
    ...
    if not adm and m.from_user.id != owner:
        return
    if len(m.command) < 2:
        return await m.reply("Gunakan format `/delbutton -100xxxx`")
    
    try:
        ids = int(m.command[1])
    except ValueError:
        return await m.reply("Format salah! ID grup harus berupa angka.")

    adm = await sub_info(c.me.id, ids)
    if adm:
        await del_sub(int(c.me.id), ids)
        await m.reply(f"{ids} telah dihapus dari Fsub")
    else:
...
async def cek_sub_bot(c, m):
    ...
    for i, ex in enumerate(admins, 1):
        try:
            ...
            text = f"`{ex['sub']}` | (ID tidak valid atau bot tidak mengenali grup ini)"
        except Exception:
            continue
        msg += f"{i} › {text}\n"
    return await m.reply(msg)
